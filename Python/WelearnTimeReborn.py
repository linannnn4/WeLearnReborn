...
def Add_A_unit(cid, uid, classid, unitidx, randommode, time_interval):
    ...
    # 定义一个线程信息打印函数
    def print_task_info(task_info_inner, use_time, running_index):
        print(
            "线程ID:",
            running_index,
            "任务名称：",
            task_info_inner["location"],
            "本次任务时长：",
            use_time,
            "该小节总时长：",
            task_info_inner["learntime"],
        )
    ...
    for task in unit_info:
        if randommode == False:
            ...
            running_index += 1
            print_task_info(task, time_interval_task, running_index)
...
if __name__ == "__main__":
    ...
    if login_mode == "1":
        try:
            ...
            username = "17723646884"  # input("请输入账号: ")
            password = "2004.0129zs"  # input("请输入密码: ")
